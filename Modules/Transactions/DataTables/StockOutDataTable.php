...
class StockOutDataTable extends DataTable
{
    ...
    public function dataTable($query): DataTableAbstract
    {
        ...
        return datatables()->eloquent($query)->editColumn('action', function ($model) {
            return view('transactions::stockOut._action', compact('model'));
     ...
     */
    public function query(Stock $model): \Illuminate\Database\Eloquent\Builder
    {
        return $model->newQuery()->with('account')->where('stock_type','stockOut');
    }
    ...
    {
        return $this->builder()->setTableId('stockOut-datatable-table')->columns($this->getColumns())->minifiedAjax()->dom('Bfrtip')->orderBy(1)->buttons(Button::make('create'), Button::make('export'), Button::make('print'), Button::make('reset'), Button::make('reload'));
    }
